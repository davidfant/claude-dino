...
- **Purpose**: Registers the root plugin source, commands, and hooks with Claude Code
- **Commands**: `/dino:start`, `/dino:reset`
- **Hooks**: 7 command-hook wrappers (prompt, tool use, session lifecycle)
...
- **Behavior**: Creates a 42-line pane below, idempotent per session
