...
- **Source**: Repository root (`.`), so commands, hooks, and scripts resolve from `${CLAUDE_PLUGIN_ROOT}`
...
- **Behavior**: Creates a 42-line pane below, idempotent per exact pane title
