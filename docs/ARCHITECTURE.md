...
- **Registration**: Command hooks in `hooks/hooks.json` invoke scripts through `${CLAUDE_PLUGIN_ROOT}`
- **JSON utilities**: `scripts/utils/hook-json.sh` parses hook payload fields and serializes state
...
claude-dino/
├── .claude-plugin/
│   ├── marketplace.json     # Marketplace source registration
