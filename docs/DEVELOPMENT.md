...
`session-start.sh` writes `CLAUDE_CODE_SESSION_ID` to `CLAUDE_ENV_FILE` when Claude Code provides it, so slash commands can target the same session state as hook events.
