...
Hook commands are registered from `hooks/hooks.json` as Claude command-hook objects. In an installed plugin, Claude provides `${CLAUDE_PLUGIN_ROOT}` so the hooks can locate repository scripts regardless of the install directory.
