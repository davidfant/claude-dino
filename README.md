...
- [Bun](https://bun.sh) — used to build and run the canvas
...
The plugin runs from its installed root via `${CLAUDE_PLUGIN_ROOT}`, so commands and hooks can locate `scripts/`, `hooks/`, and `canvas/` after marketplace installation.
